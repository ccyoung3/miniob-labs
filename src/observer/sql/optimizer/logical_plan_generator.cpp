...
RC LogicalPlanGenerator::create_plan(SelectStmt *select_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  if (!select_stmt->order_bys().empty()) {
    sort_oper = make_unique<SortLogicalOperator>(std::move(select_stmt->order_bys()));
    if (*last_oper) {
...
RC LogicalPlanGenerator::create_plan(UpdateStmt *update_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  unique_ptr<LogicalOperator> update_oper(new UpdateLogicalOperator(table, update_stmt->attribute_name(), update_stmt->update_expr()->copy()));
